...
**Verification:** raster screenshots of the heavy card DOM time out — instead serve a **no-cache copy** of `card-game-29` and drive it via the preview tool's `eval`/DOM + `getBoundingClientRect` (a local deploy/serve helper on this machine syncs the folder to a temp dir and stamps a fresh `?v=`, sidestepping Dropbox/`file://` reparse issues; navigate with a `?fresh=<ts>` buster). A **guarded auto-driver** (plays South: `open16`/`pass`, pick a trump, click the first `.playable` card, choose Discard on a void; guard against the resolve-pause) can run a full hand to check the move log / scoring — the per-hand move log must balance to **29** points on a played-out hand. Mind that a **backgrounded preview tab throttles `setTimeout`**, so auto-driven hands run slowly.
