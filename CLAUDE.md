...
- **AI card play** (Part C6–C8 in `ai.js`): `aiPlayCard` receives `effectiveTrump()` (null while concealed), so the AI never wins with inert trump. The **overtake rule (C8)** is in `aiDiscard` — never trump a trick a partner is already winning unless forced (only trumps in hand); when an opponent leads, ruff to win if able. `aiShouldReveal` (C4) is the conscious ruff-reveal decision. **Card-tracking (`seen`):** `aiPlayCard`/`aiLead`/`aiFollowSuit` take an **optional trailing `seen`** arg (`{ played:Set<cardKey>, toActAfter:[seats], declarerTrump, noTrump }`, built in `game.js` `doAIPlay`); it powers several heuristics — `aiLead` **cashes an established boss** (pure helper `isBoss`), **avoids leading the declarer's own concealed trump** (`seen.declarerTrump`), and — in a genuine **No-Trump** deal only (`seen.noTrump`, #3 Phase 1) — **establishes its longest suit** by leading low (`ntLongestEstablishSuit`, dials in `AI_TUNING.play.nt`); and `aiFollowSuit` **secures a beatable partner** (concealed/NT only) and **banks the highest-POINT winner** (`highestPointCard`) when sure to win. The change is **additive**: `seen = null` (any caller that omits it) skips every new branch and reproduces the original behaviour — asserted both ways by the committed harness `card-game-29/test-ai.js`. The AI is still handed `effectiveTrump()` (not `seen.declarerTrump`) as the active trump, so it never adjudicates with inert trump. (Distilled from the AI Improvement Log: Candidates #1/#2/#4/#5a/#6 applied 2026-06-05; **#3 No-Trump strategy Phase 1 applied 2026-06-07** — staged establish-lead + `seen.noTrump` gate, Phases 2–4 pending; #5b stop-drawing-trump remains open.)
